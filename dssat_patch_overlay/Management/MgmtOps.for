...
C-----------------------------------------------------------------------
      ...
      USE FloodModule
      IMPLICIT NONE
      EXTERNAL AUTHAR, AUTPLT, CHEMICAL, FERTAPPLY, FERTLAYERS,
     &  FERT_PLACE, IPAHAR, IRRIG, OM_PLACE, OPMGMT, PADDY_MGMT,
     &  SUMVALS, TILLAGE, TIMDIF, YR_DOY
      SAVE
      ...
      INTEGER KMAX, METFER, NAP, NCHEM, NLAYR, TILLNO          
      INTEGER NHAR, NTIL, RUN, TIMDIF
      INTEGER YRDIF, YRDOY, MDATE, YRO, YRPLT, YRS, YRSIM
      INTEGER HDATE(NAPPL)
      INTEGER STGDOY(20)

      REAL FERDEPTH_ACTION, FMIXEFF, IRRAMT, TOTIR, TIL_IRR
...
C-----------------------------------------------------------------------
     ...
     &  FERTDATA)                                 !Output
      IF (TDINT_ON) THEN
        CALL TDINT_RESET_FERT_ACTION(FERTDATA)
      ENDIF
...
C-----------------------------------------------------------------------
     ...
     &    TILLVALS, TILLNO)                               !Output
      ...
      IF (TDINT_ON) THEN
        IRRAMT = 0.0
        TIL_IRR = 0.0
        IF (TDINT_IRR .GT. 1.E-6) THEN
          IRRAMT = TDINT_IRR
          TIL_IRR = TDINT_IRR
          NAP = NAP + 1
        ENDIF
        TILLVALS % TIL_IRR = TIL_IRR
        CALL PUT('MGMT','DEPIR', IRRAMT)
        CALL PUT('MGMT','IRRAMT',IRRAMT)
      ELSEIF (INDEX('AFRDPWET',IIRRI) .GT. 0 .AND. ISWWAT .EQ. 'Y')
     &  THEN
!       Calculate irrigation depth for today
     ...
     &    FLOODWAT, IIRRI, IRRAMT, NAP, TIL_IRR, TOTIR)   !Output
      ...
      ELSE
        IRRAMT = 0.0
        TIL_IRR = 0.0
        TILLVALS % TIL_IRR = 0.0
      ENDIF

      IF (TDINT_ON .AND. TDINT_N .GT. 1.E-6) THEN
        FERDEPTH_ACTION = 1.0
        METFER = 1
        PROF = 0.0
        CALL FERTLAYERS(
     &    DLAYR, FERDEPTH_ACTION, 1, METFER, NLAYR,         !Input
     &    FERTDATA % AppType, FERTDATA % FERMIXPERC,        !Output
     &    FMIXEFF, KMAX, PROF, FERTDATA % UNINCO)           !Output
        CALL FERTAPPLY(
     &    0.0, 0.0, TDINT_N, 0.0, 0.0,                      !Input
     &    FLOOD, FMIXEFF, PROF, KMAX,                       !Input
     &    FERTDATA % ADDFUREA, FERTDATA % ADDFNH4,          !I/O
     &    FERTDATA % ADDFNO3, FERTDATA % ADDOXU,            !I/O
     &    FERTDATA % ADDOXH4, FERTDATA % ADDOXN3,           !I/O
     &    FERTDATA % ADDSNH4, FERTDATA % ADDSNO3,           !I/O
     &    FERTDATA % ADDUREA, FERTDATA % ADDSPi,            !I/O
     &    FERTDATA % ADDSKi)                                !I/O
        FERTDATA % FERTDAY = YRDOY
        FERTDATA % FERTYPE = 1
        FERTDATA % FERDEPTH = FERDEPTH_ACTION
        FERTDATA % AMTFER(N) = FERTDATA % AMTFER(N) + TDINT_N
        FERTDATA % NAPFER(N) = FERTDATA % NAPFER(N) + 1
      ENDIF
      IF (TDINT_ON) THEN
C       Interactive actions are step-scoped, not daily recurring.
        TDINT_IRR = 0.0
        TDINT_N = 0.0
      ENDIF
...
C=======================================================================
      ...
      SUBROUTINE TDINT_RESET_FERT_ACTION(FERTDATA)
      USE ModuleDefs
      IMPLICIT NONE
      ...
      FERTDATA % ADDOXU = 0.0
      FERTDATA % ADDOXH4 = 0.0
      FERTDATA % ADDOXN3 = 0.0
      ...
      RETURN
      END SUBROUTINE TDINT_RESET_FERT_ACTION
